...
void MagnotometerThread::transformation(float uncalibrated_values[3], float* calibrated_values)    
{
...
bool MagnotometerThread::isEnabled(){
    return enabled;
}
void MagnotometerThread::enable(){
    mag->begin();
    //acc->Enable();
    mag->Enable();
    enabled = true;
}
void MagnotometerThread::turn_off(){
    disable();
    // acc->Disable();
    mag->Disable();
    enabled = false;
}
void MagnotometerThread::recalculate_bias(){
  for (int i = 0; i < 3; i++){
    if (mins[i] < maxes[i]){
...
float MagnotometerThread::getAverage(float last){
...
West = 270
*/
float MagnotometerThread::getHeading()
{
